...
module.exports = {
    meta: {
        ...
        url: 'https://github.com/MarkKragerup/weblint-eslint-security/blob/master/docs/rules/no_href_and_src_inline_xss_react.md',
        fixable: 'code',
    },
    create: function (context) {
        ...
        return {
            ...
            JSXIdentifier(node) {
                ...
                const suggestedFix = `{(${propertyValueSourceCode}).toLowerCase().replace('javascript:', '/javascript/:/')}`;

                context.report({
                    node,
                    message: `${propertyName} property value might be XSS vulnerable`,
                    fix(fixer) {
                        return fixer.replaceText(node.parent.value, suggestedFix);
                    }
                });
            },
        };
